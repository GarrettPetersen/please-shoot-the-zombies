(function () {
  ...
  function makeShapeFromCorners(THREE, corners) {
  ...
  class WorldRenderer3D {
    constructor(canvas, width, height) {
      ...
      this.scene.background = new THREE.Color(0x1b1f29);
    ...
    rebuildStaticWorld() {
      ...
      if (corners.length >= 3) {
        const shape = makeShapeFromCorners(THREE, corners);
          ...
          if (tile.tileIndex != null) {
            ...
            if (slot && (slot.type === 'window' || slot.type === 'crate')) {
              ...
              if (!this.slotColliderByIndex.has(si)) {
                ...
                slotCollider.rotation.y = -syaw + Math.PI / 2;
      ...
      if (boardImg) {
        ...
        for (const b of this.state.getBoardInstances()) {
          if (b.type === 'wall') {
            ...
            this._addBillboard(b.w, b.h, b.x, b.y, b.z, tex);
          }
        }
      }
    }
